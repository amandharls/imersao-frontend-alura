import { PERFIS } from "./profiles.js";
...
/** Catálogo com títulos de fileira personalizados por perfil. */
export function getCategoriesForProfileId(profileId) {
  const perfil = PERFIS[String(profileId)];
  const primeiroNome = perfil ? perfil.nome.split(/\s+/)[0] : "Você";

  return [
    {
      ...categories[0],
      title: `${primeiroNome}: ${categories[0].title}`,
    },
    {
      ...categories[1],
      title: `Séries para ${primeiroNome}`,
    },
    {
      ...categories[2],
      title: `${categories[2].title} de ${primeiroNome}`,
    },
  ];
}
